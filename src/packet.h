...
struct RREPHeader {
  uint32_t originNodeID;   // 4 bytes: destination for which route is provided
  uint16_t lifetime;       // 2 bytes: route lifetime
  uint8_t numHops;         // 1 byte:  Number of hops using this route
};

// Extended header for RERR (16 bytes)
struct RERRHeader {
    uint32_t originNodeID;       // 4 bytes: Node that reported the issue
    uint32_t brokenNodeID;       // 4 bytes: Broken nodeID
    uint32_t originalDestNodeID; // 4 bytes: Original destination
    uint32_t originalPacketId;   // 4 bytes: Original packetID that will have been overwritten
};

// Extended header for ACk (4 bytes)
struct ACKHeader {
    uint32_t originalPacket; // 4 bytes: If ACK required return the original message id as an ack
};
...
inline size_t serialiseACK(const ACKHeader &ack, uint8_t* buffer, size_t offset) {
    memcpy(buffer + offset, &ack.originalPacket, 4);
    offset += 4;
    return offset;
}

inline size_t deserialiseACK(const uint8_t* buffer, size_t offset, ACKHeader &ack) {
    memcpy(&ack.originalPacket, buffer + offset, 4);
    offset += 4;
    return offset;
}
